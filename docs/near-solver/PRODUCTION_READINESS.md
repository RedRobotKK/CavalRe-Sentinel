...
| X17 | First live run (designer) | "decisions: none yet" is ambiguous — cannot distinguish connected-but-quiet from silently-reconnecting; bus likely gates events behind the partner API key | **Fixed**: status report shows bus frames/reconnects/malformed with plain-language health verdict. **Partner-portal registration is now the critical-path item** |
| X18 | Security (web layer) | A browser UI on a trading system must not become a mutation surface | **Enforced by construction**: status server is GET-only (405 otherwise), binds 127.0.0.1 only, zero mutating endpoints; kill switch is unreachable from the web. Tested |
